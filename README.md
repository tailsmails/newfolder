...
packs standard files into a sequence of empty, metadata-carrying directories,
...
### 1. Zero-Byte Steganographic Storage Engine
  ...
  - **Metadata-Only Data Persistence:** Packs file segments entirely into directory 
    names inside a target folder. The generated subdirectories contain absolutely 
    no internal files, utilizing only filesystem index records for storage.
      - *Filesystem Footprint:* Offloads file storage from physical disk blocks 
        into directory block tables (such as Ext4 directory indices, NTFS Index 
        Records, or APFS directory trees).
  - **Sequential Ordering Preservation:** Automatically embeds masked index keys 
    into folder name prefixes, enabling high-speed parallel scanning and deterministic 
...
### 3. Progressive Directory Shredder Engine (GNU-Shred Compliance)

  - **Multi-Pass Directory-Entry Saturation (DES):** Overwrites directory entry names 
    in-place with randomized alphanumeric sequences of identical length, neutralizing 
    original folder-name signatures on disk.
      - *In-Place Scrambling:* Replaces directory record allocations $N$ times with 
  ...
  - **Zero-Fill Obfuscation (`-z`):** Appends an optional final pass that overwrites 
    all directory entry names with homogeneous '0' character strings.
  ...
  - **Logarithmic Name Contraction & Unlinking (`-u`):** Progressively halves the 
    character length of renamed directory entries before executing the final `rmdir` system call.
...
### 1. Packing a File
Packs a physical file into a directory structure containing empty, encrypted metadata folders.
```bash
newfolder pack <input_file> <output_dir> <seed>
```

### 2. Unpacking a File
Reconstructs the original binary file from the empty metadata folder structure.
```bash
newfolder unpack <input_dir> <output_file> <seed>
```

### 3. Secure Metadata Shredding
Shreds directory names recursively using multi-pass random overwrites, zero-fills, and unlinking.
```bash
...
- `-z`: Appends a final pass overwriting directory names with '0' blocks.
- `-u`: Activates progressive name-shrinking and deletes the directories after shredding.
...
```bash
# 1. Obfuscate file into empty directory structures
...
# name contraction, and final system directory deletion with real-time logs:
...
1.  **Block Transition Map:**
    ...
    $$\text{Physical File} \rightarrow \text{Chunking } [50\text{B}] \rightarrow \text{Feistel Index Cipher} \rightarrow \text{Stream Payload Cipher} \rightarrow \text{OS Directory Allocation}$$

2.  **Metadata Overwrite Formula:**
    Each directory entry name $N$ of original length $L$ is overwritten with random bytes from the alphanumeric charset $\Sigma$ for $J$ iterations:
    
    $$N_{j} \in \Sigma^L \quad \text{for } j \in [1, \text{passes}]$$

3.  **Destruction Cycle:**
    If the `-u` parameter is enabled, the directory entry namespace is shrunk and unlinked:
    
    $$\text{Random Scramble} \rightarrow \text{Zero-Fill } [0^L] \rightarrow \text{Logarithmic Contraction} \rightarrow \text{System Unlink } (\text{rmdir})$$
